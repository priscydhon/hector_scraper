...
inf = (y+'T'+a+'E'+b+'L'+y+'E'+m+'G'+c+'R'+r+'A'+y+'M'+'  '+y+'S'+a+'C'+b+'R'+y+'A'+m+'P'+c+'E'+r+'R'+y+'  '+'2'+y+'0'+a+'2'+b+'3'+y+'  '+m+'B'+c+'Y'+r+'  '+y+'H'+'E'+y+'C'+a+'T'+b+'O'+c+'R'+'  '+'M'+c+'A'+y+'N'+a+'U'+r+'E'+b+'L')
el=0
def Sleep(timE):
    try:
        time.sleep(timE)
...
def ospath():
    ...
    for po in range(o):
        ...
        with open('multi_log.txt', 'a') as f:
            f.write(api_id+'\n'+api_hash+'\n')
        client = TelegramClient("HectorManuel{}".format(z), api_id, api_hash)
        client.start()
        Sleep(1)
        clear()
        info()
    ...
    if xc=='y':
        ...
        if cv=='y':
            ...
            if con in ['', 'n']:
                ...
                Sleep(1)
                for d in range(v-1):
                    os.remove("HectorManuel{}.session".format(d))
...
for s in range(v):
    ...
    Sleep(1)
    client = TelegramClient("HectorManuel{}".format(x), api_id, api_hash)
    client.start()
